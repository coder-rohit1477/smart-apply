...
import { RewriteRequest } from "@/types/rewrite";
...
export async function rewriteSectionAction(request: RewriteRequest) {
  ...
  const result = await rewriteResumeSection(request, user.id);
  return result;
}

export async function analyzeSectionAction(section: string, content: string, targetRole?: string) {
  await getAuthenticatedUser();
  const result = await analyzeResumeSection(section, content, targetRole);
  return result;
}

export async function getKeywordOptimizationAction(resumeId: string, jobDescription: string) {
  const user = await getAuthenticatedUser();
  const result = await getKeywordOptimization(resumeId, user.id, jobDescription);
  return result;
}

export async function performAtsMatchAction(resumeId: string, jobDescription: string) {
  const user = await getAuthenticatedUser();
...
export async function tailorResumeAction(resumeId: string, jobDescription: string, focusArea: any) {
  const user = await getAuthenticatedUser();
